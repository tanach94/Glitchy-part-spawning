...
local function Detect_ground_under_main_part(Main_part:BasePart) -- detects if spawned part is close to ground, if yes destroy
	...
	Params.FilterDescendantsInstances = {Main_part}

	local Direction = Vector3.new(0, -5, 0)
	local RayCast = workspace:Raycast(Main_part.Position + Vector3.new(0,3,0), Direction, Params)
...
----------- Custom listener
function Part_deleted_signal.New() 
	return setmetatable({_listeners = {}}, Part_deleted_signal)
end
...
function Part_deleted_signal:Destroy() -- and then this, will remove all connections
...
function Server_random_part_spawning_oop_table.Random_part_spawning_new(Target) -- creates metatable set informations etc etc and then returns itself so i can do whatever i want with this metatable
	...
	self.OnDeleted = Part_deleted_signal.New()
	...
	self.Connections.Coroutine = coroutine.create(Local_coroutine_loop)
...
function Server_random_part_spawning_oop_table:Set_properties_of_self() -- the informations in metatable what are in part properties will set to part, simple way
